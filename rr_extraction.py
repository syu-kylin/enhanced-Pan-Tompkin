...
def main(edf_fn, select_chann_name):
    ...
    logger.info(f"RR extraction done!")
    plot_rr(rr_time_stamp_enhanced, rr_intervals_enhanced, time_stamp_corrected, rr_interval_corrected)


def plot_rr(rr_time_stamp_enhanced, rr_interval_enhanced, rr_time_stamp_corrected, rr_interval_corrected):
    """
    ...
    text0 = f"mean: {mean_enhanced:.4f}\nstd: {std_corrected:.4f}"
    axs[0].plot(rr_time_stamp_corrected[1:], rr_interval_corrected)
    if min(rr_interval_corrected) < 0.3:
        axs[0].axhline(0.2,color='r')
    axs[0].set_title(f'corrected rr intervals from enhanced Pan-Tompkin')
    axs[0].text(0.9, 0.83, text0, transform=axs[0].transAxes)
    axs[0].set_xlabel('time(s)')
    axs[0].grid()

    text1 = f"mean: {mean_enhanced:.4f}\nstd: {std_enhanced:.4f}"
    axs[1].plot(rr_time_stamp_enhanced[1:], rr_interval_enhanced)
    if min(rr_interval_enhanced) < 0.3:
        axs[1].axhline(0.2,color='r')
    axs[1].set_title(f'rr intervals from enhanced Pan-Tompkin')
    ...
    plt.savefig(f"{save_dir}/rr interval.svg")
    logger.info(f"RR intervals plot saved to {save_dir}/rr interval.svg")
    plt.close()
